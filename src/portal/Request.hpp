...
protected:
    Request_proxy(sdbus::IProxy& proxy)
        : proxy_(proxy)
    {
        proxy_.uponSignal("Response").onInterface(INTERFACE_NAME).call([this](const uint32_t& response, const std::map<std::string, sdbus::Variant>& results){ this->onResponse(response, results); });
    }
